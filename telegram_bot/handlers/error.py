"""
کنترل‌کننده خطای عمومی برای ربات تلگرام.

این ماژول تابع `error_handler` را ارائه می‌دهد که به عنوان کنترل‌کننده اصلی خطاها
در برنامه ثبت می‌شود. این تابع مسئول لاگ‌گیری دقیق خطاها و ارسال گزارش کامل
(شامل traceback) به توسعه‌دهنده از طریق یک چت تلگرامی مشخص شده، و همچنین
ارسال یک پیام عمومی به کاربر در صورت امکان است.
"""
import logging
import html
import json
